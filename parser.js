...
authModule.on('end', (sessionCookie) => 
{
	...
	var logPath = "parser-logs/" + path.basename(url);

	startDownloading(sessionCookie, url, [], logPath);
});
							...
							section.urls.forEach((url, index) => 
							{
								...
								{								
									...
									else 
									{
										...
										{
											fs.writeFile(path.join(logPath, 'download_queue.txt'), JSON.stringify(downloadQueue));
											hasExecuted = true;
										}
									}
								}).on("error", (error) => 
